...
class MoleculeVisualizationCallback(Callback):
    ...
    _REQUIRED_KEYS = {"phi_gen", "phi_real", "pos_gen", "a_soft_gen", "pos_real", "a_soft_real", "batch_vec"}

    def __init__(self, n_molecules: int = 4, bond_threshold: float = 2.0, every_n_epochs: int = 1):
    ...
    def on_validation_batch_end(
        self, trainer: Trainer, pl_module: LightningModule, outputs, batch, batch_idx: int, dataloader_idx: int = 0
    ) -> None:
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        try:
            ...
            images = {
                "val/molecules/random_gen":  render_group(random_idx, ref["pos_gen"],  ref["a_soft_gen"],  "gen"),
                "val/molecules/best_gen":    render_group(best_idx,   ref["pos_gen"],  ref["a_soft_gen"],  f"best d={nn_dists[best_idx[0]]:.2f}"),
                "val/molecules/worst_gen":   render_group(worst_idx,  ref["pos_gen"],  ref["a_soft_gen"],  f"worst d={nn_dists[worst_idx[0]]:.2f}"),
                "val/molecules/real_ref":    render_group(random_idx, ref["pos_real"], ref["a_soft_real"], "real"),
            }

            if gen_types is not None and real_types is not None:
                images["val/atom_type_dist"] = self._atom_dist_chart(gen_types, real_types)
    ...
    def _render_mol(
        ...
        for t, (color, name) in enumerate(zip(_ATOM_COLORS, _ATOM_NAMES)):
            m = types == t
            if m.any():
                ax.scatter(p[m, 0], p[m, 1], p[m, 2], c=color, s=120, label=name,
    ...
    def _atom_dist_chart(self, gen_types: torch.Tensor, real_types: torch.Tensor) -> "wandb.Image":
        import matplotlib.pyplot as plt
        from PIL import Image as PILImage

        n = len(_ATOM_NAMES)
        gen_frac  = np.array([(gen_types  == t).sum().item() for t in range(n)], dtype=float)
        real_frac = np.array([(real_types == t).sum().item() for t in range(n)], dtype=float)
