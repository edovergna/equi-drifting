...
def _infer_types_heuristic(positions: np.ndarray) -> np.ndarray:
    ...
    for i in np.where((degrees == 3) & (types == -1))[0]:
        avg = dists[i, adj[i]].mean()
        types[i] = 2 if avg < _N_AVG_BOND_MAX else 1  # N or C

    types[types == -1] = 1  # C fallback for deg ≥ 4 and anything unresolved
    return types
