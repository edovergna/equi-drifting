...
This module implements PositionGenerator, a LightningModule that combines an EGNN
...
from ..losses.pos_drift_loss import (
    TrainingDivergedException,
    compute_pos_drift_loss
)
...
class PositionGenerator(LightningModule):
    ...
    def _sample_prior_batch(
        self, batch, num_atoms: int | None = None
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Sample positions, but use real one-hot atom types from the current batch.

        If n_gen_molecules != number of real molecules:
        - fewer generated molecules: sample a random subset of real type sequences
        - more generated molecules: cycle through real type sequences
        """
        ...
        _, pos, batch_vec, dense_edge_index, atom_counts = sample_prior_batch(
            self.n_gen_molecules,
            size_values,
            size_probs,
        ...
        real_types = batch.real_atom_types.to(device=self.device, dtype=pos.dtype)
        num_types = self.generator_cfg["num_atom_types"]

        if real_types.shape[-1] != num_types:
            raise ValueError(
                f"Expected {num_types} atom-type channels, got {real_types.shape[-1]}."
            )

        if real_types.shape[0] % num_atoms != 0:
            raise ValueError(
                f"Cannot reshape real_atom_types into molecules with {num_atoms} atoms."
            )

        n_real_molecules = real_types.shape[0] // num_atoms
        real_types = real_types.reshape(n_real_molecules, num_atoms, num_types)

        if self.n_gen_molecules == n_real_molecules:
            real_idx = torch.arange(n_real_molecules, device=self.device)
        elif self.n_gen_molecules < n_real_molecules:
            real_idx = torch.randperm(n_real_molecules, device=self.device)[
                : self.n_gen_molecules
            ]
        else:
            real_idx = torch.arange(self.n_gen_molecules, device=self.device)
            real_idx = real_idx % n_real_molecules

        x_prior = real_types[real_idx].reshape(
            self.n_gen_molecules * num_atoms,
            num_types,
        )

        self._last_sampled_counts = atom_counts
        self._last_sampled_atom_probs = x_prior.detach().cpu().numpy()

        return x_prior, pos, batch_vec, dense_edge_index
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        num_atoms = self._batch_num_atoms(batch)
        gen_pos, _, gen_batch_vec = self._forward(batch, num_atoms)

        real_pos = batch.pos

        try:
            loss, stats = compute_pos_drift_loss(
                    gen_pos, real_pos, num_atoms, cfg=self.drift_cfg
                )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        """
        num_atoms = self._batch_num_atoms(batch)
        gen_pos, types, gen_batch_vec = self._forward(batch, num_atoms)

        real_pos, real_types = batch.pos, batch.real_atom_types

        val_loss, stats = compute_pos_drift_loss(
            gen_pos, real_pos, num_atoms, cfg=self.drift_cfg
        )
        ...
        return {
            ...
            "gen_atom_types": types.detach().cpu().argmax(dim=-1),
    ...
    def test_step(self, batch, batch_idx):
        ...
        test_loss, stats = compute_pos_drift_loss(
                gen_pos, real_pos, num_atoms, cfg=self.drift_cfg
        )
