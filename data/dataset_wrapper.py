...
class PretrainDatasetWrapper(MixDatasetWrapper):
    def __init__(self, max_n_vertex, *datasets) -> None:
        ...
        for dataset in datasets:
            cur_approx_lengths = [
                int(x[0]) * dataset.approx_length for x in dataset._properties
            ]
            cur_preserve = (np.array(cur_approx_lengths) <= max_n_vertex).sum()
            cur_save_ratio = cur_preserve / len(cur_approx_lengths)
            print(
                f"Dataset: {dataset.name}\t| Total: {len(cur_approx_lengths)}\t| Used: {cur_preserve}\t| Ratio: {cur_save_ratio * 100:.2f}% "
            )
    ...
    def __getattr__(self, attr):
        if attr in self.__dict__:
            ...
            raise AttributeError(
                f"'DynamicBatchWrapper'(or '{type(self.dataset)}') object has no attribute '{attr}'"
            )
