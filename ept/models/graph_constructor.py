...
class GraphConstructor(nn.Module):
    ...
    def __init__(
        ...
        pre_layers: List[Dict] = [],
        edge_embed_size: int = 0,
    ) -> None:
        super().__init__()
    ...
    def prepare(self, graph: Graph) -> Graph:
        ...
        if (
            graph.block_num_units is None
        ):  # assume each unit is a block (i.e. each unit is a single node)
    ...
    def forward(
        ...
        with torch.no_grad():
            all_edges = variadic_meshgrid(
                input1=torch.arange(
                    graph.batch_ids.shape[0], device=graph.batch_ids.device
                ),
...
class ScatterBlockFeatures(nn.Module):
    ...
    def forward(self, graph: Graph) -> Graph:
        if self.merge == "add":
            graph.unit_features = (
                graph.unit_features + graph.block_features[graph.unit2block]
            )
        elif self.merge == "concat":
            graph.unit_features = torch.cat(
                [graph.unit_features, graph.block_features], dim=-1
            )
        else:
            raise NotImplementedError(f"Merge action: {self.merge} not implemented")
        return graph
...
class RadialEdge(nn.Module):
    def __init__(
        ...
        self.eps = 1e-10
        assert not (
            self.scope == "cross" and self_loop
        ), f"Self-loop is not compatible with scope {self.scope}"
    ...
    def pre_filter(
        self, graph: Graph, all_edges: Tuple[Tensor, Tensor]
    ) -> Tuple[Tensor, Tensor]:
        row, col = all_edges
    ...
    def forward(
        ...
        dist = _block_edge_dist(
            graph.unit_pos, graph.unit2block, torch.stack([row, col], dim=1)
        )
