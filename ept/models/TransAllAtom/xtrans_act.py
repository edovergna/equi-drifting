...
from .ieconv import (EfficientIEConvLayer, IEConvLayer,
                     get_equiv_ieconv_edge_feature)
...
class XTransEncoderAct(nn.Module):
    def __init__(
        ...
        use_ieconv=False,
        zero_conv=False,
        efficient_ieconv=False,
        ieconv_share_edge_feat=False,
    ) -> None:
        super().__init__()
        ...
        self.encoder = Transformer(
            ...
            use_ieconv=use_ieconv,
            zero_conv=zero_conv,
            efficient_ieconv=efficient_ieconv,
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(d_hidden * 2, d_hidden), act_fn, nn.Linear(d_hidden, d_hidden)
        )
        ...
        if self.use_ieconv:
            ...
            if efficient_ieconv:
                ...
                for i in range(0, n_layers):
                    self.add_module(
                        f"ieconv_{i}",
                        IEConvLayer(d_hidden, d_hidden // 8, d_hidden, ieconv_d_edge),
                    )
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, mol_mask=None):
        ...
        edge_feat = torch.cat(
            [H[unit_row], H[unit_col], dis_feat, edge_attr[block_edge_id]], dim=-1
        )
        ...
        edge_vecs = edge_vec.unsqueeze(-1) * equiv_feat.unsqueeze(
            -2
        )  # [Ne, 3, d_hidden]
        H = self.node_mlp(
            torch.cat(
                [H, scatter_sum(edge_scas, unit_row, dim_size=H.shape[0], dim=0)],
                dim=-1,
            )
        )
        ...
        H_batch, H_mask = graph_to_batch_nx(
            H, batch_to_nodes, mask_is_pad=False, factor_req=8
        )
        ...
        lengths[1:] = torch.cumsum(
            scatter_sum(torch.ones_like(batch_to_nodes), batch_to_nodes), dim=-1
        )[
            :-1
        ]  # [bs]
        ...
        if self.local_mask:
            rbf_feat_batch = torch.ones(
                (bs, max_n, max_n, rbf_feat.shape[-1]),
                dtype=rbf_feat.dtype,
                device=rbf_feat.device,
            ) * float("-inf")
            rbf_feat_batch[:, 0] = 0.0
            rbf_feat_batch[:, :, 0] = 0.0
        else:
        ...
        rbf_feat_batch[
            batch_to_nodes[unit_row], self_idx[unit_row], self_idx[unit_col]
        ] = rbf_feat
        rbf_feat_batch = rbf_feat_batch.reshape(
            bs, max_n, max_n, self.n_layers, -1
        ).permute(
            3, 0, 4, 1, 2
        )  # [l, bs, h, n, n]
        ...
        # svd init
        D_batch = torch.norm(
            Z_batch.unsqueeze(1) - Z_batch.unsqueeze(2), dim=-1
        )  # [bs, n, n]
        D_batch = -D_batch
        ...
        if self.use_ieconv:
            with torch.no_grad():
                ...
                if self.ieconv_share_edge_feat:
                    if mol_mask is not None:
                        ieconv_edge_feature = torch.cat(
                            [dis_feat[edge_mask], ieconv_edge_feature], dim=-1
                        )
                    else:
                        ieconv_edge_feature = torch.cat(
                            [dis_feat, ieconv_edge_feature], dim=-1
                        )
        else:
        ...
        for i in range(self.n_layers):
            if self.use_ieconv:
                ieconv_H = self._modules[f"ieconv_{i}"](
                    H, atomic_edges, ieconv_edge_feature
                )
...
class SelfAttnLayer(nn.Module):
    ...
    def __init__(
        self, d_hidden, n_heads, layer_idx=-1, efficient=False, attn_bias=True
    ):
    ...
    def forward(self, H, V, cached_info=None, ieconv_H=None):
        ...
        V_v = (
            self.vector_v(V)
            .view(batch_size, num_nodes, 3, self.n_heads, -1)
            .transpose(-2, -3)
            .flatten(start_dim=-2)
        )
        ...
        H_res = res[:, :, :, : self.d_head].reshape(
            batch_size, num_nodes, self.d_hidden
        )
        V_res = (
            res[:, :, :, self.d_head :]
...
class GVPFFNLayer(nn.Module):
    ...
    def __init__(
        self, d_hidden, d_ffn, act_fn=nn.SiLU(), d_output=None, vector_act="none"
    ):
