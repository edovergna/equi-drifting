...
class Denoise(nn.Module):
    ...
    def __init__(
        self,
        encoder: dict,
        graph_constructor: dict,
        sigma_begin=0.01,
        sigma_end=10,
        n_noise_level=50,
        rot_sigma_begin=0.01,
        rot_sigma_end=10,
        rot_n_noise_level=50,
        noise_type="atom",
        pred_type="gradient",
        **kwargs,
    ) -> None:
        ...
        sigmas = torch.tensor(
            np.exp(np.linspace(np.log(sigma_begin), np.log(sigma_end), n_noise_level)),
            dtype=torch.float,
        )
        self.sigmas = nn.Parameter(sigmas, requires_grad=False)  # [n_noise_level]
        rot_sigmas = torch.tensor(
            np.exp(
                np.linspace(
                    np.log(rot_sigma_begin), np.log(rot_sigma_end), rot_n_noise_level
                )
            ),
            dtype=torch.float,
        )
        self.rot_sigmas = nn.Parameter(
            rot_sigmas, requires_grad=False
        )  # [n_noise_level]
    ...
    def update_global_block(self, Z, B, block_id):
        ...
        not_global = ~is_global
        centers = scatter_mean(
            Z[not_global], scatter_ids[not_global], dim=0
        )  # [Nglobal, n_channel, 3], Nglobal = batch_size * 2
        Z = Z.clone()
        Z[is_global] = centers
        return Z, not_global
    ...
    def add_noise(self, Z, B, block_id, batch_id, noise_level):
        ...
        if self.noise_type == "atom":
            ...
            Z_perturbed = Z + normal_noise_atom * used_sigmas_atom.unsqueeze(
                -1
            ).unsqueeze(-1)
            Z_perturbed = self.normalize(
                Z_perturbed, B, block_id, batch_id
            )  # Zero CoM Subspace
            ...
            normal_noise_atom = (
                normal_noise_block * used_sigmas_block.unsqueeze(-1).unsqueeze(-1)
            )[block_id]
            ...
            normal_noise_atom_trans = (
                normal_noise_block_trans * used_sigmas_block.unsqueeze(-1).unsqueeze(-1)
            )[block_id]
            normal_noise_block_rot_vec, d_log_normal_noise_block_rot_vec = (
                random_normal_so3(
                    noise_level[batch_id],
            ...
            Z_perturbed = (
                block_center_atom
                + relative_position @ normal_noise_atom_rot
                + normal_noise_atom_trans
            )
            Z_perturbed = self.normalize(
                Z_perturbed, B, block_id, batch_id
    ...
    def get_score(self, Z, Z_perturbed, block_id, batch_id, block_lengths, ctx):
        ...
        if self.noise_type == "atom":
            ...
            return (
                block_center - block_center_perturbed
            ) / used_sigmas_block.unsqueeze(-1).unsqueeze(-1), block_id
            ...
            score_trans = (
                block_center - block_center_perturbed
            ) / used_sigmas_block.unsqueeze(-1).unsqueeze(-1)
            score_rot = d_log_normal_noise_block_rot_vec.view(-1, 1, 3)

            # Arms of Force
            relative_position = (
                Z_perturbed - block_center_perturbed[block_id]
            )  # [Nu, n_channel, 3]

            # Inertia Matrix
            inertia_atom = (
                stable_norm(relative_position)
                * torch.eye(3)
                .unsqueeze(0)
                .expand(relative_position.shape[0], -1, -1)
                .to(relative_position.device)
                - relative_position.transpose(2, 1) @ relative_position
            )
    ...
    def get_loss(self, pred, label_ctx):
        ...
        if self.noise_type == "atom":
            ...
            block_moment = scatter_sum(
                atom_moment, block_id, dim=0
            )  # [Nb, n_channels, 3]

            pred_rot = torch.einsum(
                "bij, bnj -> bni", inertia_inv, block_moment
            )  # [Nb, n_channels, 3]
    ...
    def forward(
        ...
        noise_level = torch.randint(
            0, self.sigmas.shape[0], (batch_size,), device=Z.device
        )
        ...
        graph = self.graph_constructor.forward(
            ...
            unit_pos=Z,
            num_nodes=lengths,
            unit_position_ids=atom_positions,
            segment_ids=segment_ids,
            block_type=B,
            block_num_units=block_lengths,
        )
        ...
        not_global_edge = torch.logical_and(
            B[edges[0]] != self.global_block_id, B[edges[1]] != self.global_block_id
        )
        ...
        if self.pred_type == "gradient":
            ...
            pred_noise = (
                (-dy).view(-1, 1, 3).contiguous()
            )  # the direction of the gradients is where the energy drops the fastest. Noise adopts the opposite direction
